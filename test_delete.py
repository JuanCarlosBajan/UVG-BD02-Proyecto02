...
def test_delete_column():
		...
		# Act
		deleted = hbase.Delete('test', 1, 'general', None, None)
		# Assert
		assert deleted == 2
		assert hbase.tables['test'].h_files[0].rows[0].enabled == False
		assert hbase.tables['test'].h_files[0].rows[1].enabled == False
		
		assert hbase.tables['test'].h_files[1].rows[0].enabled == True
		assert hbase.tables['test'].h_files[1].rows[1].enabled == True
...
def test_delete_family():
		...
		# Act
		deleted = hbase.Delete('test', 1, None, None, None)
		# Assert
		assert deleted == 4
		assert hbase.tables['test'].h_files[0].rows[0].enabled == False
		assert hbase.tables['test'].h_files[0].rows[1].enabled == False
		assert hbase.tables['test'].h_files[1].rows[0].enabled == False
		assert hbase.tables['test'].h_files[1].rows[1].enabled == False
